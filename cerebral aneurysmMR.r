##拆分下载好的肠道菌群数据
set.seed(123)
load("D:/研究课题/肠道菌群/Gut Microbiota-cerebral aneurysm/拆分数据/exp_dat.Rdata")  ##提前运行了


setwd("D:/研究课题/肠道菌群/Gut Microbiota-cerebral aneurysm")#设置工作空间
...
dir.create("拆分数据")
setwd("D:/研究课题/肠道菌群/Gut Microbiota-cerebral aneurysm/拆分数据")
for (j in 1:n) 
{
  print(name_city[j]);
  outPath = "D:/研究课题/肠道菌群/Gut Microbiota-cerebral aneurysm/拆分数据"   ##输出路径
...
GWAS_1 <- fread('finngen_R9_I9_cerebral_ANEURYSM.gz', sep = '\t', header = TRUE)
  allSNP  <- do.call(rbind, exp_dat)
  GWAS_2 <- subset(GWAS_1,GWAS_1$rsids %in% allSNP$SNP)
  rm(GWAS_1)
  GWAS_2$PHENO<-"cerebral aneurysm"
